 ...
 * @param <D> the actual type of diagram to be used (must inherit from {@link org.oryxeditor.server.diagram.generic.GenericDiagram}); {@link #getDiagram()} will return this type
 */
public abstract class GenericNode<S extends GenericShape<S,D>, D extends GenericDiagram<S,D>> extends GenericShapeImpl<S,D> {
	 ...
	 * 
	 * @param p
	 * @throws org.oryxeditor.server.diagram.exception.TooManyDockersException if the node already has a docker (nodes may only have one docker)
	 */
	@Override
	 ...
	 * @throws org.oryxeditor.server.diagram.exception.TooManyDockersException if p is not null and the node already has a docker (nodes may only have one docker)
	 */
	@Override
